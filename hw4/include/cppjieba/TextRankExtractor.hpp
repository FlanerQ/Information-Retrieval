      ...
      public:
        ...
        void rank(WordMap &ws, size_t rankTime = 10) {
            ...
            for (Graph::iterator edges = graph.begin(); edges != graph.end();
                 ++edges) {
                // edges->first start节点；edge->first end节点；edge->second
                // 权重
            ...
            for (size_t i = 0; i < rankTime; i++) {
                for (NodeSet::iterator node = nodeSet.begin();
            ...
            for (WordMap::iterator i = ws.begin(); i != ws.end(); i++) {
                ws[i->first].weight = (i->second.weight - min_rank / 10.0) /
                                      (max_rank - min_rank / 10.0);
            }
        }
    };

  public:
    TextRankExtractor(const string &dictPath, const string &hmmFilePath,
    ...
    void Extract(const string &sentence, vector<Word> &keywords, size_t topN,
                 size_t span = 5, size_t rankTime = 10) const {
        vector<string> words;
        ...
        for (size_t i = 0; i < words.size(); i++) {
            ...
            for (size_t j = i + 1, skip = 0;
                 j < i + span + skip && j < words.size(); j++) {
        ...
        for (WordMap::iterator itr = wordmap.begin(); itr != wordmap.end();
             ++itr) {
