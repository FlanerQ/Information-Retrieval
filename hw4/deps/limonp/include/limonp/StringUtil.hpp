...
template <class T> string Join(T begin, T end, const string &connector) {
    string res;
...
inline vector<string> Split(const string &src, const string &pattern,
                            size_t maxsplit = string::npos) {
    vector<string> res;
...
void Unicode32ToUtf8(Uint32ContainerConIter begin, Uint32ContainerConIter end,
                     string &res) {
    res.clear();
    uint32_t ui;
    while (begin != end) {
...
void UnicodeToUtf8(Uint16ContainerConIter begin, Uint16ContainerConIter end,
                   string &res) {
    res.clear();
    uint16_t ui;
    while (begin != end) {
    ...
    while (i < len) {
        if (0 == (str[i] & 0x80)) {
            ...
            if (i + 1 < len) { //&& (str[i+1] & 0x80))
                uint16_t tmp = (((uint16_t(str[i]) & 0x00ff) << 8) |
                                (uint16_t(str[i + 1]) & 0x00ff));
                vec.push_back(tmp);
                i += 2;
            } else {
                return false;
...
template <class Uint16ContainerConIter>
void GBKTrans(Uint16ContainerConIter begin, Uint16ContainerConIter end,
              string &res) {
    res.clear();
    // pair<char, char> pa;
    char first, second;
...
inline void GetTime(const string &format, string &timeStr) {
    ...
    size_t len = strftime((char *)timeStr.c_str(), timeStr.size(),
                          format.c_str(), &tmNow);
