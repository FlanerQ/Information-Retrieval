    ...
    if(fs.is_open()){
        ...
        while(std::getline(fs,doc)){    //read text by line
            ...
            while(ss>>word){    //use stringstream to split string to word 
                inverted_index[parse_word(word)].push_back(line);   //Put this word into the inverted index list
            }
            line++;
            ss.clear();
        }
...
std::list<int> NOT(std::list<int> a){
    std::list<int> res;
    for(int i=1;i<=docs_num;i++) res.push_back(i);
