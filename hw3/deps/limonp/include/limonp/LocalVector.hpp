...
using namespace std;
/*
 * LocalVector<T> : T must be primitive type (char , int, size_t), if T is
 * struct or class, LocalVector<T> may be dangerous.. LocalVector<T> is simple
 * and not well-tested.
 */
...
template <class T> class LocalVector {
  ...
  public:
    ...
    LocalVector(const_iterator begin,
                const_iterator end) { // TODO: make it faster
        init_();
        while (begin != end) {
            push_back(*begin++);
        }
  ...
  public:
    LocalVector<T> &operator=(const LocalVector<T> &vec) {
        ...
        if (vec.buffer_ == vec.ptr_) {
            memcpy(static_cast<void *>(buffer_), vec.buffer_,
                   sizeof(T) * size_);
            ptr_ = buffer_;
        } else {
...
template <class T> ostream &operator<<(ostream &os, const LocalVector<T> &vec) {
    if (vec.empty()) {
        return os << "[]";
    }
    os << "[\"" << vec[0];
