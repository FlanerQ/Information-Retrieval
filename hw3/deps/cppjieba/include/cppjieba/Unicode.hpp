...
struct Word {
    ...
    Word(const string &w, uint32_t o, uint32_t unicode_offset,
         uint32_t unicode_length)
...
// [left, right]
inline Word GetWordFromRunes(const string &s, RuneStrArray::const_iterator left,
                             RuneStrArray::const_iterator right) {
...
inline vector<Word> GetWordsFromWordRanges(const string &s,
                                           const vector<WordRange> &wrs) {
    vector<Word> result;
...
inline void GetStringsFromWords(const vector<Word> &words,
                                vector<string> &strs) {
