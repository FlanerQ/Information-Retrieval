...
//AND
std::vector<INDEX_POS> AND(std::map<int,std::vector<int>>& a,std::map<int,std::vector<int>>& b,std::string s){
    int interval=0;
...
int main(){
    ...
    while(std::cin>>s1>>s2>>s3){
        std::vector<INDEX_POS> q=AND(inverted_index[s1],inverted_index[s2],s3);
        if(q.size()==0) std::cout<<"No match"<<std::endl;
        for(auto& i:q){
            std::cout<<"docID: "<<i.docID<<" pos1: "<<i.pos1<<" pos2: "<<i.pos2<<std::endl;
        }
        std::cout<<std::endl;
    }
}
